...
export default function StudentPerformancePage() {
  ...
  useEffect(() => {
    ...
    if (trendRef.current && myEx.length > 0) {
      ...
      trendInst.current = new Chart(ctx, {
        ...
        options: { responsive: true, maintainAspectRatio: false, scales: { x: { grid: { display: false } }, y: { grid: { color: 'rgba(42,42,58,.3)' }, min: 0, max: 100, ticks: { callback: v => v + '%' } } }, plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ctx.parsed.y + '%' } } } },
      })
    }
  ...
  if (loading) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'var(--t3)' }}>Loading performance data...</div>
  }

  if (!data) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'var(--err)' }}>Failed to load performance data.</div>
  }
  ...
  // Calculate Grade Distribution and legend count
  const gd = { Ap: 0, A: 0, Bp: 0, B: 0, C: 0, F: 0 }
  myEx.forEach((r: any) => {
    if (r.grade === 'A+') gd.Ap++
    else if (r.grade === 'A') gd.A++
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '8fr 4fr', gap: 20, marginBottom: 20 }}>
        ...
        <div className="card si si-3">
          ...
          <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {legend.map(l => <div key={l.grade} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12 }}><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ width: 10, height: 10, borderRadius: 2, background: l.color }} /><span style={{ color: 'var(--t2)' }}>{l.grade}</span></div><span style={{ color: 'var(--t1)', fontWeight: 600 }}>{l.count}</span></div>)}
          </div>
        </div>
      </div>
            ...
            <div style={{ width: 140 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ flex: 1, height: 8, borderRadius: 4, background: 'var(--bg2)' }}>
                  <div style={{ height: '100%', borderRadius: 4, width: e.pct + '%', background: getGradeColor(e.pct), transition: 'width .8s' }} />
                </div>
