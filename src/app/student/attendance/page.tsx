...
export default function StudentAttendancePage() {
  ...
  if (loading) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'var(--t3)' }}>Loading attendance details...</div>
  }

  if (!data) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'var(--err)' }}>Failed to load attendance details.</div>
  }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '5fr 7fr', gap: 20 }}>
        <div className="card si si-2">
          ...
          <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[['Present', pr, '#10B981'], ['Late', la, '#F0B429'], ['Absent', ab, '#EF4444']].map(([label, count, color]) => (
              <div key={String(label)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ width: 10, height: 10, borderRadius: 2, background: String(color) }} /><span style={{ color: 'var(--t2)' }}>{String(label)}</span></div>
                <span style={{ color: 'var(--t1)', fontWeight: 600 }}>{String(count)} days</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ overflowY: 'auto', maxHeight: 320 }}>
            <table className="dt">
              ...
              <tbody>
                ...
                {myAt.length === 0 && (
                  <tr><td colSpan={2} style={{ textAlign: 'center', padding: 24, color: 'var(--t3)' }}>No attendance records found</td></tr>
                )}
              </tbody>
            </table>
          </div>
